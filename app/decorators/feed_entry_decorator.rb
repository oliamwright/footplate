class FeedEntryDecorator < Draper::Base
  ...
  def publish_button(application = nil, remote = true)
    text = application ? application.to_s : 'all'

    if !feed_entry.in_scheduler || (application && !feed_entry.publish_to[application])
      h.button_to "Publish to #{text}", h.publish_feed_entry_path(feed_entry, app: application), remote: remote
    else
      h.button_to "Unpublish from #{text}", h.unpublish_feed_entry_path(feed_entry, app: application), remote: remote
    end
  end
